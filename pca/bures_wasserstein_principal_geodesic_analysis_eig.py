...
def compute_vec(mean, eig, angle):
    rotation = make_rotation_2d(angle)
    return rotation @ np.diag(eig) @ rotation.T @ mean


def project_on_line(points, mean, eig, angle):
    vec = compute_vec(mean, eig, angle)
    times = np.stack([np.sum((pt - mean) * vec) for pt in points])
    lbd_min, lbd_max = np.sort(eig)
...
def check_boundary_2d(points, mean, eig, angle):
    vec = compute_vec(mean, eig, angle)
    times = np.stack([np.sum((pt - mean) * vec) for pt in points])
    lbd_min, lbd_max = np.sort(eig)
    time_inf = - 1 / lbd_max * (lbd_max > 0) - 1e6 * (lbd_max <= 0)
    time_sup = - 1 / lbd_min * (lbd_min < 0) + 1e6 * (lbd_min >= 0)
    # eig_sorted = np.sort(eig)
    # if np.prod(eig_sorted) < 0.:
    #     t_inf = - 1 / eig_sorted[1]
    #     t_sup = - 1 / eig_sorted[0]
    # elif eig_sorted[0] < 0.:
    #     t_inf = - np.inf
    #     t_sup = - 1 / eig_sorted[0]
    # else:
    #     t_inf = - 1 / eig_sorted[1]
    #     t_sup = np.inf
    bool = ((times < time_inf) + (times > time_sup))
    n_outliers = np.sum(bool)
    return n_outliers
...
class BuresWassersteinPGA2D:
    def __init__(
        ...
        self.sq_roots = None
        self.points = None
        self.point_angles = None
        self.means = None
        self.time = None
        self.vecs = None
        self.vec_eigs = None
        self.vec_angles = None
        self.fiber_angles = None
        self.success = np.zeros(3)
        self.time_constraints_1 = None
        self.time_constraints_2 = None
        self.time_constraints_3 = None

    def cost_func(self, n):
        """Cost function of component n_component.
        """
        return cost_func(self.points, self.means[n], self.vec_eigs[n], self.vec_angles[n])
    ...
    def component(self, n):
        ...
        component = compute_component(self.points, self.means[n], self.vec_eigs[n], self.vec_angles[n])
        boundary_check = check_boundary_2d(self.points, self.means[n], self.vec_eigs[n], self.vec_angles[n])
    ...
    def component_init(self, n):
        ...
        if n == 0:
            ...
            self.point_angles = np.zeros(n_points)
            self.points = points_from_angles_2d(self.point_angles, self.sq_roots)

            mean_0 = np.sum(self.points, axis=0) / n_points
            self.means = np.stack([mean_0, np.zeros((2, 2)), np.zeros((2, 2))])

            vec_angle_0 = np.random.uniform(low=0., high=2*np.pi)
            vec_eig_0 = np.random.rand(2)
            vec_0 = compute_vec(mean_0, vec_eig_0, vec_angle_0)
            vec_eig_0 /= norm(vec_0)
            vec_0 = compute_vec(mean_0, vec_eig_0, vec_angle_0)
            assert np.abs(norm(vec_0) - 1) < 1e-5
            self.vec_eigs = np.stack([vec_eig_0, np.zeros(2), np.zeros(2)])
            self.vec_angles = np.array([vec_angle_0, 0., 0.])
            self.vecs = np.stack([vec_0, np.zeros((2, 2)), np.zeros((2, 2))])
        elif n == 1:
            self.fiber_angles = np.zeros(2)
            self.time = 0.
            self.means[1] = self.means[0].copy()

            vec_eig_1 = np.random.rand(2)
            vec_angle_1 = np.random.uniform(low=0., high=2 * np.pi)
            vec_1 = compute_vec(self.means[1], vec_eig_1, vec_angle_1)
            vec_1 = vec_1 - np.sum(vec_1 * self.vecs[0]) * self.vecs[0]
            self.vecs[1] = vec_1 / norm(vec_1)
            self.vec_eigs[1], eigenvec = np.linalg.eigh(self.vecs[1] @ np.linalg.inv(self.means[1]))
            self.vec_angles[1] = np.arctan(- eigenvec[0, 1] / eigenvec[1, 1])
            # assert np.abs(np.sum(self.vecs[0] * self.vecs[1])) < 1e-5
            # assert np.abs(np.sum(self.vecs[1] ** 2) - 1) < 1e-5
            # assert norm(compute_vec(self.means[1], self.vec_eigs[1], self.vec_angles[1]) - self.vecs[1]) < 1e-5
        else:
            self.means[2] = self.means[1].copy()

            vec_aux = np.random.rand(2, 2)
            vec_2 = (vec_aux + vec_aux.T) @ self.means[2]
            vec_0 = self.vecs[0] @ make_rotation_2d(self.fiber_angles[0])
            vec_1 = self.vecs[1]
            vec_2 = vec_2 - np.sum(vec_2 * vec_0) * vec_0 - np.sum(vec_2 * vec_1) * vec_1
            self.vecs[2] = vec_2 / norm(vec_2)
            self.vec_eigs[2], eigenvec = np.linalg.eigh(self.vecs[2] @ np.linalg.inv(self.means[2]))
            self.vec_angles[2] = np.arctan(- eigenvec[0, 1] / eigenvec[1, 1])
            # assert np.abs(np.sum(self.vecs[2] * vec_1)) > 1e-5:
            # assert np.abs(np.sum(self.vecs[2] * vec_2)) < 1e-5
            # assert np.abs(np.sum(self.vecs[2] ** 2) - 1) < 1e-5
            # rotation = make_rotation_2d(self.vec_angles[2])
            # assert norm(rotation @ np.diag(self.vec_eigs[2]) @ rotation.T @ self.means[2] - self.vecs[2]) < 1e-5
    ...
    def component_step_1(self, n):
        ...
        def func2min(x):
            ...
            return cost_func(points, self.means[n], self.vec_eigs[n], self.vec_angles[n])

        h = scipy.optimize.minimize(func2min, x0=self.point_angles)
        self.point_angles = h['x']
        self.points = points_from_angles_2d(self.point_angles, self.sq_roots)
    ...
    def component_0_step_2(self):
        ...
        """
        def func2min(x):
            return cost_func(self.points, x[:4].reshape((2, 2)), x[4:6], x[-1])

        def unit_norm_constraint(x):
            vec = compute_vec(x[:4].reshape((2, 2)), x[4:6], x[-1])
            return np.sum(vec ** 2) - 1

        def time_constraints(x):
            mean = x[:4].reshape((2, 2))
            vec = compute_vec(mean, x[4:6], x[-1])
            times = np.stack([np.sum((pt - mean) * vec) for pt in self.points])
            lbd_min, lbd_max = np.sort(x[4:6])
            time_inf = - 1 / lbd_max * (lbd_max > 0) - 1e6 * (lbd_max <= 0)
            time_sup = - 1 / lbd_min * (lbd_min < 0) + 1e6 * (lbd_min >= 0)
        ...
        x0 = np.hstack([self.means[0].reshape(4), self.vec_eigs[0], self.vec_angles[0]])
        h = scipy.optimize.minimize(func2min, x0=x0, constraints=cons)
        x_sol = h['x']
        self.success[0] = 1 * h['success']
        self.means[0] = x_sol[:4].reshape((2, 2))
        self.vec_eigs[0] = x_sol[4:6]
        self.vec_angles[0] = x_sol[-1]
        self.vecs[0] = compute_vec(self.means[0], self.vec_eigs[0], self.vec_angles[0])
        self.time_constraints_1 = time_constraints(x_sol)
    ...
    def component_1_step_2(self):
        ...
        def func2min(x):
            ...
            mean = (self.means[0] + time * self.vecs[0]) @ fiber_rotation
            return cost_func(self.points, mean, vec_eig, vec_angle)

        def unit_norm_constraint(x):
            time = x[1]
            vec_eig = x[2:4]
            vec_angle = x[-1]
            vec_rotation = make_rotation_2d(np.squeeze(vec_angle))
            mean_translated = self.means[0] + time * self.vecs[0]
            vec_translated = vec_rotation @ np.diag(vec_eig) @ vec_rotation.T @ mean_translated
            return np.sum(vec_translated ** 2) - 1

        def orthogonality_constraint(x):
            time = x[1]
            vec_eig = x[2:4]
            vec_angle = x[-1]
            rotation = make_rotation_2d(np.squeeze(vec_angle))
            mean_translated = self.means[0] + time * self.vecs[0]
            vec_translated = rotation @ np.diag(vec_eig) @ rotation.T @ mean_translated
            return np.sum(self.vecs[0] * vec_translated)
        ...
        def time_constraints(x):
            ...
            fiber_rotation = make_rotation_2d(np.squeeze(fiber_angle))
            mean = (self.means[0] + time * self.vecs[0]) @ fiber_rotation
            vec = compute_vec(mean, vec_eig, vec_angle)
            times = np.stack([np.sum((pt - mean) * vec) for pt in self.points])
            lbd_min, lbd_max = np.sort(vec_eig)
            time_inf = - 1 / lbd_max * (lbd_max > 0) - 1e6 * (lbd_max <= 0)
            time_sup = - 1 / lbd_min * (lbd_min < 0) + 1e6 * (lbd_min >= 0)
            return np.hstack((times - time_inf, time_sup - times))
        ...
        x0 = np.hstack([self.fiber_angles[0], self.time, self.vec_eigs[1], self.vec_angles[1]])
        h = scipy.optimize.minimize(func2min, x0=x0, constraints=cons)
        x_sol = h['x']
        self.success[1] = 1 * h['success']
        self.fiber_angles[0] = x_sol[0]
        self.time = x_sol[1]
        self.means[1] = (self.means[0] + self.time * self.vecs[0]) @ make_rotation_2d(self.fiber_angles[0])
        self.vec_eigs[1] = x_sol[2:4]
        self.vec_angles[1] = x_sol[-1]
        self.vecs[1] = compute_vec(self.means[1], self.vec_eigs[1], self.vec_angles[1])
        self.time_constraints_2 = time_constraints(x_sol)
    ...
    def component_2_step_2(self):
        ...
        def func2min(x):
            ...
            vec_eig = x[1:3]
            vec_angle = x[-1]
            mean = self.means[1] @ make_rotation_2d(np.squeeze(fiber_angle))
            return cost_func(self.points, mean, vec_eig, vec_angle)
        ...
        def unit_norm_constraint(x):
            ...
            vec_angle = x[-1]
            rotation = make_rotation_2d(np.squeeze(vec_angle))
            vec_translated = rotation @ np.diag(vec_eig) @ rotation.T @ self.means[1]
            return np.sum(vec_translated ** 2) - 1
        ...
        def orthogonality_constraints(x):
            ...
            vec_angle = x[-1]
            rotation = make_rotation_2d(np.squeeze(vec_angle))
            sym = rotation @ np.diag(vec_eig) @ rotation.T
            fiber_rotation = make_rotation_2d(self.fiber_angles[0])
            return np.stack((
                np.sum((sym @ self.means[1]) * (self.vecs[0] @ fiber_rotation)),
                np.sum((sym @ self.means[1]) * self.vecs[1])
            ))
        ...
        x0 = np.hstack([self.fiber_angles[1], self.vec_eigs[2], self.vec_angles[2]])
        h = scipy.optimize.minimize(func2min, x0=x0, constraints=cons)
        x_sol = h['x']
        self.success[2] = 1 * h['success']
        self.fiber_angles[1] = x_sol[0]
        self.vec_eigs[2] = x_sol[1:3]
        self.vec_angles[2] = x_sol[-1]
        self.means[2] = self.means[1] @ make_rotation_2d(self.fiber_angles[1])
        self.vecs[2] = compute_vec(self.means[2], self.vec_eigs[2], self.vec_angles[2])
        self.time_constraints_3 = time_constraints(x_sol)
    ...
    def fit(self, points_spd):
        ...
        mean_spd = project(self.means[1])
        vecs_spd = np.stack([
            tangent_project(self.vecs[0] @ make_rotation_2d(self.fiber_angles[0]), self.means[1]),
            tangent_project(self.vecs[1], self.means[1]),
            tangent_project(self.vecs[2], self.means[2])
        ])
        costs, variances = evaluate_results(components, points_spd, mean_spd)
        costs, variances, components, status, message = postprocess_2d(costs, variances, components, boundary_checks)

        return {
            'costs': costs, 'variances': variances, 'components': components,
            'mean_spd': mean_spd, 'vecs_spd': vecs_spd, 'status': status, 'message': message,
            'boundary_checks': boundary_checks, 'costs_gl': costs_gl,
            'time_constraints_1': self.time_constraints_1, 'time_constraints_2': self.time_constraints_2,
            'time_constraints_3': self.time_constraints_3,
        }
